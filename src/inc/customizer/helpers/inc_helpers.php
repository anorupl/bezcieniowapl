 ...
 *
 * @since	1.0.1
 * @param	string	$prim_mod_key
 * @param	string	$mod_key
 *
 * @return	string with html
 */
 function wpg_the_phone(  $primary_phone = 'wpg_telephone' , $secondary_phone = 'wpg_phone' ) {

	$primary_phone   = get_theme_mod($primary_phone, '');
	$secondary_phone = get_theme_mod($secondary_phone, '');

	if ($primary_phone !== '') : ?>
		<i class="item-icon icon-phone_android"></i><?php _e('Call us ', 'wpg_theme');
		//print primary_phone
		printf('<span><a href="tel:%1$s">%2$s</a></span> | ',
			str_replace(' ','', $primary_phone),
			esc_html($primary_phone)
		);

		//print secondary_phone
		$secondary_phone = explode("|", $secondary_phone);

		foreach ($secondary_phone as $value) {
			printf('<span><a href="tel:%1s">%2$s</a></span> | ',
			str_replace(' ','', $value),
			esc_html($value));
		}
	endif;
