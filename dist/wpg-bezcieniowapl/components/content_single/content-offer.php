...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<div class="entry-content col-7" style="float:right;">
		...
		?>
			<div class="col-6">
				<a href="<?php echo (get_post_meta( get_the_ID(), 'wpg_gallery_offer', true ) !== '') ? get_term_link(intval(get_post_meta( get_the_ID(), 'wpg_gallery_offer', true ))) : '#' ?>" class="relation-link"><?php _e('Show Gallery','wpg_theme'); ?></a>							
			</div>
		<?php		
		if (get_theme_mod('wpg_telephone', '') !== '') : ?>
				<div class="col-6">
	...
	<?php if ( has_post_thumbnail() ) { ?>
		<figure class="entry-image post-thumbnail col-5" style="float: left;">
				<?php the_post_thumbnail( 'full', array( 'alt' => get_the_title() ) ); ?>
		</figure><!-- .entry-image -->
	<?php }	?>	
	<?php if (is_single()) : ?>
		<div class="entry-gallery col-5">
 			<?php
